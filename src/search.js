 ...
 */
function createSearchHandler(Model, options) {
  return (routeOptions = {}) => async (request, reply) => {
    ...
    if (routeOptions && routeOptions.filter && typeof routeOptions.filter === 'function') {
      queryParameters = {
        ...queryParameters,
    ...
    if (projection) {
      if (typeof projection === 'string' && projection.trim().startsWith('{')) {
    ...
    if (routeOptions.projection && typeof routeOptions.projection === 'function') {
